...
public class DaemonClientTest {
  ...
  /**
   * A cancel-ack for the in-flight request must reach the caller.
   *
   * <p>It is addressed to the cancel's own id -- a cancel is its own request -- so the read
   * loop's id filter drops it, and a caller waiting to hear that its cancel landed hears nothing
   * until the run itself ends. That is what turned "the daemon accepted the cancellation
   * immediately" into "the daemon did not acknowledge the cancellation in time": the shutdown
   * hook was waiting for the task, not for the answer. {@code targetId} is what makes the ack
   * recognisable to the request it abandons.
   */
  @Test
  public void aCancelAckForThisRequestReachesTheCaller() throws IOException {
    ...
    servePeer(s -> {
      try {
        ...
        s.out().write(Frame.doneFrame("a-quite-different-id", 0));
        assertTrue("the ack must have been seen before the request finished",
            awaitQuietly(acked));
        s.out().write(Frame.doneFrame(requestId, 3));
      } catch (IOException e) {
        throw new RuntimeException(e);
      }
    });
