...
public abstract class Definition extends UserDataHolderImpl implements CoreDefinition {
  ...
  /**
   * The instances the typechecker picked while checking this definition, recorded as it picked
   * them. They cannot be read back off the core afterwards: an instance whose call does not
   * survive elaboration leaves no trace there, so an importer told to go by the core alone
   * removes the import that supplied it.
   *
