        ...
        if (!collector.isEmpty()) {
          ...
          try {
            ...
            withTCDefLock(() -> {
              typecheckingIndicator.checkCanceled();
              if (checkerFactory == null) {
                // Drop the previously stored typechecking errors before re-reporting the fresh ones,
                // so that stale errors that are no longer produced (e.g. an error that disappeared
                // once the code was fixed, possibly after being broken and restored) do not linger in
                // the error map. Errors that are still present are reported again right afterwards.
                if (definitions == null) {
                  // Whole-module typechecking: clear every typechecking error of the modules that
                  // have at least one definition re-typechecked in this run. This also removes errors
                  // stored under referables that are not part of the resolved definitions (e.g.
                  // auxiliary definitions generated by metas during typechecking), which the
                  // resolution phase cannot reset. It is safe because, within a module that is being
                  // re-typechecked, every definition that still has a typechecking error is itself
                  // re-typechecked in this run (a changed module is re-resolved, and definitions with
                  // lingering typechecking errors are forced to be re-typechecked during resolution),
                  // so all genuine errors are reported again below. Modules whose definitions are all
                  // up-to-date (nothing in the collector) keep their errors untouched.
                  Set<ModuleLocation> modulesToClear = new HashSet<>();
                  for (CollectingOrderingListener.Element element : collector.getElements()) {
                    for (Concrete.ResolvableDefinition definition : element.getAllDefinitions()) {
                      ModuleLocation module = definition.getData().getLocation();
                      if (module != null) modulesToClear.add(module);
                    }
                  }
                  for (ModuleLocation module : modulesToClear) {
                    myServer.getErrorService().clearTypecheckingErrors(module);
                  }
                } else {
                  // Targeted typechecking of specific definitions: only reset the definitions that
                  // are actually re-typechecked, to avoid dropping errors of other definitions in
                  // the same module that are not re-checked in this run.
                  for (CollectingOrderingListener.Element element : collector.getElements()) {
                    for (Concrete.ResolvableDefinition definition : element.getAllDefinitions()) {
