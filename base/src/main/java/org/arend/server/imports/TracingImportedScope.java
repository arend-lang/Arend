 ...
 */
final class TracingImportedScope implements TracedScope {
  ...
  private TracingImportedScope(Scope prelude, ImportedScope imported, List<ConcreteNamespaceCommand> imports, List<String> consumed, NamespaceCommandUsage usage) {
    myPrelude = prelude;
    myImported = imported;
    myImports = imports;
    myConsumed = consumed;
  ...
  static @NotNull TracingImportedScope forFile(@NotNull ConcreteGroup group, @NotNull ModuleScopeProvider provider, @NotNull NamespaceCommandUsage usage) {
   ...
   */
  private @Nullable ConcreteNamespaceCommand findImport(List<String> path) {
  ...
  private NamespaceCommandUsage.@Nullable Part partFor(String name) {
    List<String> path = new ArrayList<>(myConsumed);
    path.add(name);
    ConcreteNamespaceCommand command = findImport(path);
    return command == null ? null : NamespaceCommandUsage.partOf(command, name);
  }
  ...
  public @Nullable Referable resolveName(@NotNull String name, @Nullable ScopeContext context) {
    ...
    if (referable != null) {
      NamespaceCommandUsage.Part part = partFor(name);
    ...
    return myElements.computeIfAbsent(context, ctx -> {
      ...
      if (myPrelude != null) {
        myPrelude.find(referable -> {
          result.putIfAbsent(referable.textRepresentation(), new Entry(referable, null, null));
          return false;
        }, ctx);
      }
      myImported.find(referable -> {
        String name = referable.textRepresentation();
        result.computeIfAbsent(name, n -> new Entry(referable, null, partFor(n)));
        return false;
      }, ctx);
      return result;
    });
  ...
  public @Nullable Referable find(Predicate<Referable> pred, @Nullable ScopeContext context) {
    ...
    for (Entry entry : getEntries(context).values()) {
  ...
  private final class Descent implements Scope {
    ...
    private NamespaceCommandUsage.@Nullable Part partForPath(String name) {
    ...
    public @Nullable Referable resolveName(@NotNull String name, @Nullable ScopeContext context) {
      ...
      if (referable != null) {
        NamespaceCommandUsage.Part part = partForPath(name);
        if (part != null) myUsage.charge(part);
      }
      return referable;
    }
