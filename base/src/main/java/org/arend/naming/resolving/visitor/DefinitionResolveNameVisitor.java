...
public class DefinitionResolveNameVisitor implements ConcreteResolvableDefinitionVisitor<Scope, Void>, DocVisitor<Scope, Void> {
  ...
  public void resolveGroup(ConcreteGroup group, Scope scope, ArendInstances instances, Map<LongName, DefinitionData> definitionData) {
    ...
    for (ConcreteStatement statement : statements) {
      ...
      Scope importedScope = namespaceScope.getCommandPathSubScope(namespaceCommand);
    ...
    record NamespaceStruct(boolean isStatic, ConcreteNamespaceCommand command, Map<String, Referable> refMap) {
      static void addNamespaceStruct(Scope.ScopeContext context, ConcreteNamespaceCommand cmd, Scope namespaceScope, List<NamespaceStruct> result) {
        Collection<? extends Referable> elements = NamespaceCommandNamespace.resolveNamespace(namespaceScope.getCommandPathSubScope(cmd), cmd).getElements(context);
