 ...
 *
 * <p>Reading is deliberately not serialised, and is not reachable from here at all: the field is
 * write-only, so there is no accessor that could hand a caller a read end this class cannot
 * guarantee. One connection has one reader by construction ({@code Frame.read} on the socket
 * itself), and a lock shared with the writer would deadlock the moment a reader blocked.
 */
