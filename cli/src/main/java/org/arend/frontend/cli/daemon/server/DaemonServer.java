...
public final class DaemonServer {
  ...
  private void handleRequest(FrameChannel ch, Map<String, Object> req) throws IOException {
    ...
    switch (op) {
      ...
      case "cancel" -> {
        // A queued request is cancellable too: the client cannot see the queue, so refusing
        // to cancel something the worker has simply not reached yet is arbitrary from there.
        String target = String.valueOf(req.getOrDefault("targetId", ""));
        boolean accepted = !target.isEmpty()
            && (target.equals(currentTaskId) | workQueue.removeIf(item -> target.equals(item.requestId())));
      ...
      case "shutdown" -> {
        ...
        workQueue.add(new WorkItem(SHUTDOWN_SENTINEL, id, null, null));
      }
      case CLI_OP -> {
        String[] args = extractArgs(req);
        workQueue.add(new WorkItem(CLI_OP, id, args, ch));
      }
      case REFRESH_OP -> {
