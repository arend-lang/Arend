 ...
 *       invocation (typically {@code -L <libdir> <libRef>}).</li>
 ...
 *   <li>Write the lock file with the bound socket address (signals READY to the parent) --
 *       after the hook, so a signal in between cannot leave the lock behind.</li>
...
public final class DaemonMain {
  ...
  public static void run(String[] args) {
    ...
    boolean synthetic = "--daemon-bootstrap-synthetic".equals(args[0]);
    String libRef = args[1];
    String[] forwarded = Arrays.copyOfRange(args, 2, args.length);

    DaemonPaths paths = synthetic
        ? DaemonPaths.resolveSynthetic(Paths.get(libRef))
        : DaemonPaths.resolve(Paths.get(libRef));
    if (paths == null) {
    ...
    // The per-thread stdout/stderr shim is installed by DaemonServer.start(), which is where it
    // has to happen anyway (a server that cannot answer is not a state worth leaving reachable
    // by forgetting a call). A second call here would be harmless but would suggest the first
    // one is not enough.
