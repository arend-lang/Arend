...
public final class CliDispatcher {
  ...
  /**
   * Rewrites relative values of path-valued options to the client's directory.
   *
   * <p>Only {@code --log-file} needs it: it is the one option a daemon-served command still
   * honours whose value is a path this process will open. The other path-valued options are all
   * in {@link LockedFlags} -- fixed at bootstrap and ignored per request -- so rebasing them
   * would change nothing, and the positionals are handled where they are classified, by
