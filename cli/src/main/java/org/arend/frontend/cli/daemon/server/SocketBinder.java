 ...
 * the daemon. It is loopback-only and last-resort, and {@link #bind} says so on stderr -- with
 * the reason the socket could not be bound, which is what distinguishes "another daemon already
 * holds it" from "this filesystem cannot host one" -- rather than leaving the difference
 * undocumented at the point where it matters.
 */
  ...
  public static Bound bind(Path udsPath) throws IOException {
    ...
    if (!isWindows()) {
      ...
      try {
        ...
        // Keep the reason: "Address already in use" means another daemon holds the socket, which
        // is a different problem -- and a different fix -- from a filesystem that cannot host one.
        udsFailure = e.getMessage() == null ? e.toString() : e.getMessage();
      } catch (UnsupportedOperationException e) {
        udsFailure = "Unix domain sockets are not supported here";
      }
    }
