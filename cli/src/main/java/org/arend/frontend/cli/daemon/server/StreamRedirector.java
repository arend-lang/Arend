...
public final class StreamRedirector {
  ...
  /**
   * Undoes {@link #install()}, putting back the streams it replaced.
   *
   * <p>{@code install()} is a process-wide side effect with no natural end, which is fine in a
   * daemon JVM that exits when the daemon does and wrong everywhere else: a test that starts a
   * server leaves every later test in the same JVM writing through the shim, and anything that
   * saves and restores {@code System.out} around its own work -- {@code JsonOutputRedirect}
   * does exactly that -- then restores the shim instead of the real stream.
   */
  ...
  public static final class FrameOutputStream extends OutputStream {
     ...
     */
    private static final int MAX_CHUNK_CHARS = Frame.MAX_FRAME_BYTES / 8;

    /**
     * Sends {@code data}, split across frames if it is too long for one.
     *
     * <p>Splitting is safe and refusing is not: the client appends every {@code data} verbatim,
     * so a frame is a piece of the stream rather than a line of it. Handing an oversize body to
     * {@link FrameChannel#write} instead raises an IOException into a {@link PrintStream}, which
     * records it in its trouble flag and tells nobody -- so the output simply disappears, and
     * only for a command that happened to be routed through a daemon.
     */
    private void sendFrame(String data) throws IOException {
      ...
      do {
        ...
        client.write("stdout".equals(kind)
            ? Frame.stdoutFrame(requestId, chunk)
            : Frame.stderrFrame(requestId, chunk));
        start = end;
