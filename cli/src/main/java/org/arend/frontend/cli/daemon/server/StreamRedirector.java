...
public final class StreamRedirector {
   ...
   */
  public static FrameOutputStream attach(SocketChannel client, String requestId) {
  ...
  /** Flush partial-line buffers and clear the current thread's redirect. */
  public static void detach() {
    try {
      OutputStream o = outTl.get();
      if (o instanceof FrameOutputStream f) f.flushLine();
      OutputStream e = errTl.get();
      if (e instanceof FrameOutputStream f) f.flushLine();
   ...
   * Writes to the underlying {@link SocketChannel} are synchronized so concurrent
   * writers can't interleave bytes within a single frame.
   */
  public static final class FrameOutputStream extends OutputStream {
    ...
    FrameOutputStream(SocketChannel client, String requestId, String kind) {
      this.client = client;
    ...
    private void sendFrame(String data) throws IOException {
      Map<String, Object> body = "stdout".equals(kind)
          ? Frame.stdoutFrame(requestId, data)
          : Frame.stderrFrame(requestId, data);
      synchronized (client) {
        Frame.write(client, body);
      }
    }
