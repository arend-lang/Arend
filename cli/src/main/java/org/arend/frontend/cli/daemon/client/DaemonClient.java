...
public final class DaemonClient implements AutoCloseable {
  ...
  public int invoke(String id, String op, Map<String, Object> extra,
    ...
    return invoke(id, op, extra, onFrame, null);
  }

  /**
   * As {@link #invoke(String, Map, Consumer)}, but also reports the daemon's answer to a
   * {@code cancel} this client sent for this request -- which nothing else can observe, because
   * the ack is addressed to the cancel and this loop is waiting on the request.
   */
  public int invoke(String id, String op, Map<String, Object> extra,
    ...
    while (true) {
      ...
      // The ack for a cancel we sent, before the id filter drops it: it is addressed to the
      // cancel's own id, so nothing downstream would ever see it, and a caller waiting to hear
      // that its cancel landed would instead wait out the whole run and report a timeout for a
      // cancellation the daemon took immediately. targetId names the request, which is this one.
