...
public final class ConsoleHelp {
  ...
  static void printGrouped(Options cmdOptions, List<String> diagnosticOptions) {
    ...
    // Read off LockedFlags rather than restated here: a second copy of the list in prose is a
    // copy that goes stale the first time the set changes.
    ConsoleHelpRenderer.printWrapped("Daemon-served commands lock these to the daemon's bootstrap "
        + "values; a per-request value is parsed, warned about and ignored (use --no-daemon to "
        + "override): "
        + LockedFlags.NAMES.stream().map(LockedFlags::display).collect(Collectors.joining(", "))
        + ".", 0, width);
    ConsoleHelpRenderer.printWrapped("These are rejected outright inside a daemon-served command: "
        + "-i, -d, --daemon-stop, --daemon-ping, --daemon-status, --daemon-refresh.", 0, width);
    ConsoleHelpRenderer.printWrapped("-i and -s are never routed to a daemon in the first place: "
        + "the REPL is interactive, and -s names the sources to typecheck, which a daemon would "
        + "ignore in favour of its own library. Both always run in this process.", 0, width);
  }
