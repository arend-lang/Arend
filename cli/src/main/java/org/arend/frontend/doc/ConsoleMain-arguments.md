...
- After that an ordinary `arend LIBRARY ...` is **routed to the daemon automatically**; the
  output and exit code are the same as running in this process. `--no-daemon` opts out.
...
- Per-daemon state lives in `<library>/.arend/`: `daemon.lock`, `daemon.sock`, `daemon.log`.
  The directory and socket are created private to the owner, which is the whole of the access
  control — anything that can reach the socket can run commands against the library.
- Ctrl-C on a routed command cancels the work on the daemon and exits 130.
