...
class ReplaceLongNameWithShortNameIntention: SelfTargetingIntention<ArendLongName>(ArendLongName::class.java, ArendBundle.message("arend.import.replaceWithShortName")) {
  ...
  override fun applyTo(element: ArendLongName, project: Project, editor: Editor) {
    ...
    companion object {
       fun isApplicableTo(l : ArendRefIdentifier): Boolean {
         val server = l.project.service<ArendServerService>().server
         val scope = l.ancestor<ArendDefinition<*>>()?.tcReferable?.let { server.getReferableScope(it) }
         return l.resolve is PsiLocatedReferable && scope?.resolveName(l.text).let { it == null || (it as? LocatedReferableImpl)?.data == l.resolve }
       }
    }
}
