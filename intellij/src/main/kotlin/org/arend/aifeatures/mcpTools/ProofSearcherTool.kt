...
class ProofSearcherTool : McpTool {
  ...
  private fun executeProofSearch(project: Project, query: String): String {
    ...
    val resultString = resultsList.joinToString("\n") { entry ->
      val file = entry.def.containingFile.virtualFile
      val path = file?.path
      val isReadOnly = file?.fileSystem is JarFileSystem
      val readOnlyTag = if (isReadOnly) " [READ-ONLY]" else ""
      "${entry.def.refName} at $path$readOnlyTag"
    }
