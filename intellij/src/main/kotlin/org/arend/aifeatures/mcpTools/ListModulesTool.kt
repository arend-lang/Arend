...
//            println("descriptionMap = $descriptionMap")
      ...
      for ((libName, libModules) in modulesByLibrary.toSortedMap()) {
        val libConfig = project.findLibrary(libName)
        val sourcesDirFile = libConfig?.sourcesDirFile
        val isJar = sourcesDirFile?.fileSystem is JarFileSystem
        val isExternal = libConfig?.isExternalLibrary() ?: true
        val libTag = when {
          isJar -> " (library JAR)"
        ...
        for (module in libModules.sortedBy { it.toString() }) {
          ...
          val readOnlyTag = if (isJar) " [READ-ONLY]" else ""
          val moduleFullPath = sourcesDirFile?.findFileByRelativePath(modulePathString.replace(".", "/") + ".ard")?.path
          val pathInfo = if (moduleFullPath != null) " (path: $moduleFullPath)" else ""

          if (moduleDescription != null){
            result.appendLine("  ${module.modulePath}$readOnlyTag$pathInfo (description: $moduleDescription) (${module.locationKind})")
          } else {
            result.appendLine("  ${module.modulePath}$readOnlyTag$pathInfo (${module.locationKind})")
          }
        }
      }
    }
