...
// LibraryLoadOrderTest calls ConsoleMain.loadLibraryWithDependencies, which exists only in
// staging's CLI wiring. This branch kept its own ConsoleMain, where library loading lives in
// CliSetup instead. Drop this exclude once the org.arend.frontend.query tools are ported over
// and the two ConsoleMain implementations are reconciled.
sourceSets {
    test {
        java {
            exclude("**/LibraryLoadOrderTest.java")
        }
    }
}
