...
public class EqualitySolver extends BaseEqualitySolver {
  ...
  private EqualitySolver(BaseEquationMeta meta, ExpressionTypechecker typechecker, ConcreteFactory factory, ConcreteReferenceExpression refExpr, CoreClassDefinition forcedClass, boolean useSolver) {
    super(meta, typechecker, factory, refExpr, null);
    this.forcedClass = forcedClass;
    this.useSolver = useSolver;
  }

  public EqualitySolver(BaseEquationMeta meta, ExpressionTypechecker typechecker, ConcreteFactory factory, ConcreteReferenceExpression refExpr, CoreClassDefinition forcedClass) {
    this(meta, typechecker, factory, refExpr, forcedClass, true);
  }

  public EqualitySolver(BaseEquationMeta meta, ExpressionTypechecker typechecker, ConcreteFactory factory, ConcreteReferenceExpression refExpr, boolean useSolver) {
    this(meta, typechecker, factory, refExpr, null, useSolver);
  }

  public EqualitySolver(BaseEquationMeta meta, ExpressionTypechecker typechecker, ConcreteFactory factory, ConcreteReferenceExpression refExpr) {
  ...
  public ConcreteExpression solve(@Nullable ConcreteExpression hint, @NotNull TypedExpression leftExpr, @NotNull TypedExpression rightExpr, @NotNull ErrorReporter errorReporter) {
    ...
    ValuesRelationClosure closure = new ValuesRelationClosure(values, new EquivalenceClosure<>(factory.ref(typechecker.getPrelude().getIdpRef()), factory.ref(meta.inv), factory.ref(meta.concat), factory));
    if (useHypotheses) {
  ...
  private boolean initializeAlgebraSolver(CoreExpression type) {
    ...
    Pair<TypedExpression, CoreClassCallExpression> pair = Utils.findInstanceWithClassCall(new InstanceSearchParameters() {
      ...
      }, meta.carrier, type, typechecker, refExpr, forcedClass);
    if (pair != null) {
