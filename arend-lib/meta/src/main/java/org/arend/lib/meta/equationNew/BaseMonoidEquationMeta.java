...
public abstract class BaseMonoidEquationMeta extends BaseMetaDefinition {
  ...
  protected abstract CoreClassDefinition getMonoid();
  ...
  public @Nullable TypedExpression invokeMeta(@NotNull ExpressionTypechecker typechecker, @NotNull ContextData contextData) {
    ...
    Pair<TypedExpression, CoreClassCallExpression> instance = Utils.findInstanceWithClassCall(new SubclassSearchParameters(getMonoid()), carrier, equality.getDefCallArguments().getFirst(), typechecker, marker, getMonoid());
    if (instance == null) {
      return null;
    }
    ...
    List<TermOperation> operations = Arrays.asList(
        new TermOperation(ideTerm, FunctionMatcher.makeFieldMatcher(instance.proj2, instance.proj1, getIde(), typechecker, factory, marker, 0)),
        new TermOperation(mulTerm, FunctionMatcher.makeFieldMatcher(instance.proj2, instance.proj1, getMul(), typechecker, factory, marker, 2))
    );
