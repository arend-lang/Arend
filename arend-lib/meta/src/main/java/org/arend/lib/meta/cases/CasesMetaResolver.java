...
public class CasesMetaResolver implements MetaResolver {
  ...
  private ConcreteExpression resolveArgument1(ExpressionResolver resolver, ConcreteExpression argument) {
    ...
    if (sequence.size() >= 3 && sequence.getLast().isExplicit() && sequence.get(sequence.size() - 2).isExplicit()) {
    ...
    if (sequence.size() >= 2 && sequence.getLast().isExplicit()) {
      ConcreteExpression lastArg = resolveArgRef(resolver, sequence.getLast().getExpression());
  ...
  public @Nullable ConcreteExpression resolvePrefix(@NotNull ExpressionResolver resolver, @NotNull ContextData contextData) {
    ...
    if (args.size() > 1) {
      ...
      resolveDefaultClause(resolver, null, args.getFirst().getExpression());
    }
    return builder.build();
  }

  class ArgParameters {
    final ConcreteExpression expression;
    final ConcreteExpression type;
    final boolean addPath;
    final ArendRef as;
    final ArendRef name;
